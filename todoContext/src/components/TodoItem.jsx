...
function TodoItem({todo}) {
  ...
  return (
    <div
        className={`felx border border-black/10 rounded-lg px-3 py-1.5 gap-x-3 shadow-sm shadow-white/50 duration-300 text-black ${ todo.completed ? "bg-[#c6e9a7]":"bg-[#ccbed7]"}`}>
            <input 
            type='checkbox'
            ...
            <button className="inline-flex w-8 h-8 rounded-lg text-sm border border-black/10 justify-center items-center bg-gray-100 shrink-0" onClick={()=> deleteTodo(todo.id)}>
            ❌
            </button>
